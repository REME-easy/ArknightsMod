...
public class TargetArrow {
    ...
    public static void open(AbstractCreature _source, TargetType _targetType) {
        source = _source;
    ...
    public static void open(Vector2 s, TargetType _targetType) {
        source = null;
        from = s;
        targetType = _targetType;
        canBePlayer = false;
    ...
    private static void use(Hitbox hitbox) {
        for(TargetArrowSubscriber subscriber : subscribers) {
            subscriber.receiveTargetHitbox(hitbox);
        }
    }

    public static void update() {
        if(isActive) {
//            if (InputHelper.justClickedRight || AbstractDungeon.player.hoveredCard != null || CInputActionSet.cancel.isJustPressed()) {
...
//            }
            ...
            if(targetType == TargetType.Friend) {
                for(AbstractOperator o : OperatorGroup.GetOperators()) {
                    if(o.hb.hovered && !o.isDying) {
                        hoveredCreature = o;
                        break;
                    }
                }

                if(canBePlayer && hoveredCreature == null) {
                    if(AbstractDungeon.player.hb.hovered)
                        hoveredCreature = AbstractDungeon.player;
                }
            }else if(targetType == TargetType.Monster){
                ...
                for(Hitbox h : OperatorGroup.hitboxes) {
                    if(h.hovered) {
                        hoveredHitbox = h;
                        break;
                    }
            ...
            if(InputHelper.justClickedLeft || CInputActionSet.select.isJustPressed()) {
                ...
                if(targetType != TargetType.Hitbox) {
                    ...
                    if(hoveredHitbox != null) {
                        use(hoveredHitbox);
                        logger.info("点到了");
                        logger.info("targetArrow.isActive:" + isActive);
                        logger.info("hoveredHitbox:" + hoveredHitbox);
                        close();
                        GameCursor.hidden = false;
                    }
                }
    ...
    public interface TargetArrowSubscriber extends ISubscriber {
        ...
        void receiveTargetHitbox(Hitbox hitbox);
