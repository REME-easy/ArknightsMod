...
public class TargetArrowScreen implements TargetArrow.TargetArrowSubscriber {
    ...
    public void open(String header, TargetArrow.TargetType type) {
        timer = 0.1F;
        AbstractDungeon.player.cardInUse.target_x += 200.0F * Settings.scale;
        TargetArrow.open(new Vector2(Settings.WIDTH / 2.0F, 0.0F), type);
        this.isActive = true;
...
//    }
    ...
    public void update() {
        ...
        TargetArrow.update();
        TargetArrow.from.x = AbstractDungeon.player.cardInUse.current_x;
        TargetArrow.from.y = AbstractDungeon.player.cardInUse.current_y;
    ...
    public void render(SpriteBatch sb) {
        ...
        if(this.isActive && header != null) {
            FontHelper.renderDeckViewTip(sb, header,96.0F * Settings.scale, Settings.CREAM_COLOR);
        }
    }
    ...
    @Override
    public void receiveTargetHitbox(Hitbox hitbox) {
        for(TargetArrowScreenSubscriber subscriber : subscribers) {
            subscriber.receiveScreenTargetHitbox(hitbox);
        }
        subscribers.clear();
    }
    ...
    public interface TargetArrowScreenSubscriber extends ISubscriber {
        ...
        void receiveScreenTargetHitbox(Hitbox hitbox);
